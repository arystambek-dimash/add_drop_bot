...
def setup_middlewares(dispatcher: Dispatcher,
    ...
    dispatcher.message.middleware(RedisSessionMiddleware(redis_repo, student_repo))
    dispatcher.callback_query.middleware(RedisSessionMiddleware(redis_repo, student_repo))


def register_routers(dp: Dispatcher, redis_repo: IRedisRepository, student_repository: IStudentRepository):
    router = Router()
    """ start router """
    setup_start_routes(router, student_repository)
    """ auth router """
    setup_auth_routes(router, student_repository)
    """ menu router """
    setup_menu_routes(router, student_repository)
    """ transcript router """
    setup_transcript_routes(router, student_repository, redis_repo)
    """ profile router """
    setup_profile_routes(router, student_repository, redis_repo)
    """ student router """
